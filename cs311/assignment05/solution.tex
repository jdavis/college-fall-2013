...
\begin{homeworkProblem}
    ...
    The algorithm also assumes that the queue works such that when initialized
    with the array, the elements are popped off in an increasing order based on
    the index of the array.

    \begin{algorithm}[]
        \begin{algorithmic}[1]
            \Function{BST-Reconstruction}{$traversal$}
                \State{} $partition \gets$ new \Call{Queue}{$traversal$}
                \State{} \Return \Call{BST-Reconstruction-Rec}{$Q, null$}
            \EndFunction{}
            \\
            \Function{BST-Reconstruction-Rec}{$Q, parent$}
                \State{} $root \gets$ new \Call{Node}{$Q.dequeue()$}
                \\
                \If{$Q.size() \neq 0$}
                    \If{$root.value \geq Q.peek()$}
                        \State{} $root.left \gets$ new \Call{BST-Reconstruction-Rec}{$Q, root.value$}
                    \EndIf{}
                \Else
                    \State{} \Return{root}
                \EndIf{}
                \If{$Q.size() \neq 0$}
                    \If{$parent \geq Q.peek()$ or $parent == null$}
                        \State{} $root.right \gets$ new \Call{BST-Reconstruction-Rec}{$Q, parent$}
                    \EndIf{}
                \Else
                    \State{} \Return{root}
                \EndIf{}
...
\begin{homeworkProblem}
    ...
    The definition of a connected graph is one in which every vertex is
...
\begin{homeworkProblem}
    ...
    The algorithm works as follows. \alg{AddMST} will look at which vertex
    the new edge is being connected to. Then in order to maintain
