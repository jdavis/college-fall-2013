...
\begin{homeworkProblem}
    ...
    The reason a single source vertex in a dag means it is a \textit{mother
    vertex} is because if more than one exist, than that means that the \(n\)
    source vertices can never reach each other and thus it isn't a
    \textit{mother vertex}.
    \\

    \textbf{Reference}

    Algorithm idea inspired by Problem 5:
