...
\begin{homeworkProblem}
    ...
    three edges straight across the line of nodes, from \(s\) to \(t\) to \(u\)
    then to \(v\). However the second graph is Figure~\ref{fig:graph2} has had
    \(k\) added to each edge, the shortest path is now the single edge from
    \(s\) to \(v\).
    \\
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[->,>=stealth,node distance=3cm,very
            ...
            (s) edge node [above=4mm, right] {1} (t)
                edge [bend right] node [above=4mm, right] {4} (v)
            (t) edge node [above=4mm, right] {1} (u)
            (u) edge node [above=4mm, right] {1} (v);
        \end{tikzpicture}
        \caption{The graph \(G\).}
        \label{fig:graph1}
        ...
        \begin{tikzpicture}[->,>=stealth,node distance=3cm,very
            ...
            (s) edge node [above=4mm, right] {6} (t)
                edge [bend right] node [above=4mm, right] {9} (v)
            (t) edge node [above=4mm, right] {6} (u)
            (u) edge node [above=4mm, right] {6} (v);
        \end{tikzpicture}
        \caption{The graph \(G'\).}
        \label{fig:graph2}
    \end{figure}
