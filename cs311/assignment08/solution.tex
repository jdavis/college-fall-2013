...
\begin{homeworkProblem}
    ...
    \solution
        To prove that the Longest Path problem is in
        NP-complete, we must first show the following:
...
\begin{homeworkProblem}
    ...
    + \log_{10} n\) for any positive integer \(n\).  The number of values
    actually tested for \alg{PrimalityTesting($n$)} is thus
\end{homeworkProblem}
