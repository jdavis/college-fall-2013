...
\begin{homeworkProblem}
    ...
    \begin{proof}
        ...
        We wish to show for a AVL tree with a depth of \(d + 1\) and a balance
        factor of 0, it still creates a ``full'' binary tree in that at every
        depth there are \(2^{d + 1}\) nodes.
        \\
        ...
        depth of \(d + 1\) because the new root adds one to the depth.
        \\
        ...
        \[
            \begin{split}
                \mbox{nodes at level } d &= \mbox{nodes at level d of } T_1 + \mbox{nodes at level d of } T_2
            \end{split}
        \]

        By assuming the induction hypothesis and get the following:

        \[
            \begin{split}
                \mbox{nodes at level } d &= 2^d + 2^d
                \\
                &= 2 \cdot 2^d
                \\
                &= 2^{d + 1}
        ...
        Thus at every depth of a AVL tree with a depth of \(d + 1\) and a
        balance factor of 0 still has \(2^{d}\) or 0 nodes.
    \end{proof}
\end{homeworkProblem}
