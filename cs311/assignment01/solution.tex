...
\begin{homeworkProblem}
    ...
    \textbf{Part D}
    Prove \textbf{Part C} using induction for all \(n > 3\).

    \begin{proof}
        We will prove that a single light switch can be toggled using \(n\) number of switches
        where \(n > 3\).
        \\

        \textbf{Base} Our base case will consist of two bases, when \(n = 4\) and \(n = 5\).
        \\

        In the first case, when \(n = 4\), the circuit is simple
    \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}
    ...
    \(C = \left\{ {1}\right\} \). Which gives:

    \[
        \begin{split}
            (A - B) \cup C &= (\emptyset - \emptyset) \cup {1}
            \\
            {1} &= (A \cup B) - C
            \\
            {1} &= (\emptyset \cup \emptyset) - {1}
            {1} &= \emptyset - {1}
            {1} &\neq \emptyset
        \end{split}
    \]
...
\begin{homeworkProblem}
    ...
    Given the number of coins is 100, to solve this, we will use induction on
    the number of original heads, \(n\) that we knew existed at the beginning
    of the problem. The proposition is as follows:
    \\

    \textbf{Proposition} Prove that by splitting the coins into two groups of
    50 and then flipping \(n\) number of coins in one group and adding them
    to the other group, will result in an equal number of heads in each group.

    \begin{proof}
        \textbf{Base} To make a more comprehensive base case, we will do when
        \(n = 0\) and \(n = 1\).

        When \(n = 0\), after splitting when we are automatically done. Since
        there are no heads in the entire group, there will be no heads in the
        two separate groups of 50.
        \\

        When \(n = 1\), after splitting we can have two possible outcomes:
        either the heads is in the group we will flip, or it is in the group we
        will add to. If the heads is in the group we alter, we have two more
        possible outcomes either the 1 coin we choose to flip will either be a
        heads, or it will be a tails.
        \\

        \textbf{Step} We will now show that given \(n\) number of heads,
        we can come up with an algorithm to end up with equal number of
        heads in each of the two groups when the number of heads is increased
        by one, or \(n + 1\).
        \\

        When given \(n\) coins, we assume the induction hypothesis and know
        that we can split up the coins into two groups, each with equal number
        of heads.
        \\

        If an adversary were to pick a coin at random from either group and
        flip it.  We know that we would only have to take one coin from either
        pile, flip it, and then add it to the opposite pile.
        \\

        This is because without loss of generality, the coin that we take will either be a
        heads or a tails. If it is a heads, then that group will have one less heads
        \\

        Err, I need to fix the induction step...
