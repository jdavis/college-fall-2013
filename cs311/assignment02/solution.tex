...
\begin{homeworkProblem}
    ...
    \begin{proof}
        We will show that \(f + g \in O(g)\) by using the definition of Big-Oh.
        Suppose that \(f \in O(g)\). That means \(\exists c_0 \exists n_0
        ...
        \(c_1 = c_0 + 1\). Since this proves what wished to, our proof is
        complete.
    \end{proof}
    ...
    \begin{proof}
        To prove that \(O(f + g) = O(g)\), we need to show that \(O(f + g)
        \subseteq O(g)\) and \(O(g) \subseteq O(f + g)\) because Big-Oh
        notation creates a set of functions.
        \\

        \textbf{Part One}

        First we must show that \(O(f + g) \subseteq O(g)\) First
    ...
    \begin{proof}
        ...
        \[
            \begin{split}
                ...
                &= 4^{n + 1} - 1
                \\
                &= 4 \cdot 4^1 \cdot 4^2 \hdots 4^{n - 1} \cdot 4^{n} - 1
                \\
                &= (4 - 1) \cdot 4^1 \cdot 4^2 \hdots 4^{n - 1} \cdot 4^{n}
                \\
                &= 3 \cdot 4^1 \cdot 4^2 \hdots 4^{n - 1} \cdot 4^{n}
                \\
                &= 3k
            \end{split}
        \]

        where \(k \in \mathbb{Z}\). Thus it is divisible by three.
