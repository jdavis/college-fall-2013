...
(define (run program-string . argument-strings)
  (set! DEBUG #f)
  (initialize-pc!)
  (run-helper program-string argument-strings)
  )

(define (run-debug program-string . argument-strings)
  (set! DEBUG #t)
  (initialize-pc!)
  (run-helper program-string argument-strings))

(define (run-helper program-string . argument-strings)
  (define (create-arg-sym index)
    (string->symbol (string-append "arg" (number->string index))))
  (define prev-debug-val DEBUG)
  (define (get-args)
...
(define (debug ast env)
  (if DEBUG
      (begin
        (bump-pc!)
        (debug-ast ast env (std-printing-fun "s"))
        (let ((val (value-of-indirection ast env)))
          (begin
            (bump-pc!)
            (debug-ast ast env (std-printing-fun "e"))
            val)))
      (value-of-indirection ast env)
      )
  )
