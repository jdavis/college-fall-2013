...
;============================== tests problem 2 ================================
;===============================================================================
;you are NOT required to write tests for garbage collection, these are only
;suggestion of how you could do this.
(define p2
  (test-suite
   ...
   (test-case
    ...
    (check-equal?
     (begin
       ;to help yourself you can write imperative test of the form:
       (initialize-store!)
       ;(newref (num-val 42))
       ;(newref (num-val 42))
       ;(mark-all)
       ;;or
       ;(clear-reachable some-manually-constructed-environment)
       ;and so forth
       the-store
       )
     '();replace '() with a list representing the expected state of the store after
       ;the test has executed. I have not provided these tests as they would be highly
       ;depenedant on how you choose to modify the representation of the store.
     )
    )
   )
  )
